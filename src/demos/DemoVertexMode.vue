...
// function buildUniqueEdgesFromIndexedGeometry(
//   g: THREE.BufferGeometry,
//   vertexToGroup: number[],
// ) {
...
function buildFaceGroups(g: THREE.BufferGeometry, vertexToGroup: number[]) {
  ...
  const sortEdge = (a: number, b: number): [number, number] =>
    a < b ? [a, b] : [b, a];
  ...
  const areCoplanar = (a: number, b: number) => {
    ...
    return (
      ta.normal.dot(tb.normal) > 1 - planeEpsilon &&
      Math.abs(ta.planeConstant - tb.planeConstant) < planeEpsilon
    );
  };
    ...
    while (queue.length) {
      ...
      for (const edge of tri.edges) {
        ...
        for (const neighbor of edgeToTriangles.get(key) ?? []) {
          if (neighbor === current || triangleToGroup[neighbor] !== -1)
            continue;
...
function updateEdgeOverlayPositions(
  mesh: THREE.Mesh,
  entries: EdgeOverlayEntry[],
...
onMounted(() => {
  ...
  const groupedVertices = buildCoincidentVertexGroups(
  ...
  // edgePairs = buildUniqueEdgesFromIndexedGeometry(
  //   mesh.geometry as THREE.BufferGeometry,
  //   groupIndexByVertexIndex,
  // );

  const groupedFaces = buildFaceGroups(
    mesh.geometry as THREE.BufferGeometry,
    groupIndexByVertexIndex,
  );
  ...
  const onDown = (ev: PointerEvent) => {
    ...
    if (mode.value === "vertex") {
      ...
      selectedVertexGroup =
        coincidentVertexGroups[groupIndexByVertexIndex[hit.index]];
      selectedEdgeGroups = null;
      selectedFaceGroupIndex = null;
    ...
    if (mode.value === "edge") {
      ...
      for (const edge of faceGroup.boundaryEdges) {
        ...
        const closest = new THREE.Line3(start, end).closestPointToPoint(
          localHitPoint,
          true,
          new THREE.Vector3(),
        );
  ...
  <div style="width: 800px">
    <div style="margin-bottom: 8px">
      ...
      <div style="margin-top: 8px">
        当前模式：{{
          mode === "vertex" ? "点模式" : mode === "edge" ? "线模式" : "面模式"
        }}
      </div>
    </div>
